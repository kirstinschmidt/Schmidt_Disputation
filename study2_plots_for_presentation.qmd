...
sign_profiles <- mplus_output_sig$parameters$unstandardized %>% 
  ...
  geom_errorbar(aes(ymin = est - se, ymax = est + se),
    ...
    geom_point(position = position_dodge(width = 0.2),
             size = 2.5) +
    theme_ipsum_ps(axis_text_size = "10", ticks = TRUE)+
  ...
  ggtitle("Signifikanz-Stichprobe ") +
...
cou3_profiles <- mplus_output_cou3$parameters$unstandardized %>% 
  ...
  theme(legend.position = "bottom",
        ...
        plot.title = element_text(hjust = 0.5),
        legend.box = "vertical") +
  ggtitle("Cohen's "~U[3]~"-Stichprobe") +
  guides(color = guide_legend(nrow = 2, byrow = TRUE),
         group = guide_legend(nrow = 2, byrow = TRUE),
         shape = guide_legend(nrow = 2, byrow = TRUE),
         linetype = guide_legend(nrow = 2, byrow = TRUE))
...
ggsave("LatentProfiles.png", combined_profiles, bg='transparent', width = 8.5, height = 4, dpi = 400)
